...
function node(overrides = {}) {
  return {
    ...
    clickable: false, enabled: true, editable: false, sensitive: false, depth: 4,
    ...overrides,
  };
}

const home = { package: 'com.android.settings', revision: 10, nodes: [
...
const deviceInfo = { package: 'com.android.settings', revision: 11, nodes: [
  ...
  node({ text: 'Память', resource_id: 'android:id/title' }), node({ text: '165 ГБ / 256 ГБ', resource_id: 'android:id/summary' }),
  node({ text: 'Процессор', resource_id: 'android:id/title' }), node({ text: 'Qualcomm®\nSnapdragon™\n782G Octa-core', resource_id: 'com.android.settings:id/assignment' }),
  node({ text: 'Батарея', resource_id: 'android:id/title' }), node({ text: '5000 мА·ч (ТИП)', resource_id: 'com.android.settings:id/assignment' }),
  node({ text: 'RAM', resource_id: 'android:id/title' }), node({ text: '12,0 ГБ +', resource_id: 'com.android.settings:id/coui_statusText1' }),
  ...
  node({ text: 'Ядро системы', resource_id: 'android:id/title' }), node({ text: 'Trinity Engine', resource_id: 'com.android.settings:id/assignment' }),
...
test('parses read-only device information from semantic nodes', () => {
  assert.deepEqual(parseSettingsDeviceInfo(deviceInfo), {
    model_name: 'OnePlus Nord CE3 5G', system_version_display: '15.0', storage: '165 ГБ / 256 ГБ',
    processor: 'Qualcomm®\nSnapdragon™\n782G Octa-core', battery_capacity: '5000 мА·ч (ТИП)',
    ram: '12,0 ГБ +', ram_expansion: '4,00 ГБ', front_camera: 'Фронтальная 16MP',
    back_camera: 'Основная 50MP+8MP+2MP', system_core: 'Trinity Engine', display_size: '17,02 см',
  });
});
...
test('skill only launches Settings, scrolls, opens About device, then completes', async () => {
  ...
  const forbidden = await skill.validateDirective({ state: SettingsState.DEVICE_INFO, snapshot: deviceInfo, directive: { type: 'CLICK_HANDLE', handle: 'anything', purpose: 'CHANGE_SETTING' } });
