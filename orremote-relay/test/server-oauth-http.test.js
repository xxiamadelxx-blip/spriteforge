...
function stubDeviceRelay() {
  return {
    ...
    presence() { return { deviceId: 'a'.repeat(24), connected: false, sessionEpoch: null }; },
    async forwardMcp() { throw new Error('device forwarding not expected in this test'); },
  };
}
...
async function start(t) {
  const cfg = config();
  const server = createRelayServer(cfg, stubDeviceRelay());
  server.listen(0, '127.0.0.1');
  await once(server, 'listening');
  t.after(() => server.close());
  return { cfg, base: `http://127.0.0.1:${server.address().port}` };
}

test('public OAuth metadata is available at both RFC 9728 resource paths', async (t) => {
  const { cfg, base } = await start(t);
  ...
  assert.equal(metadata.issuer, cfg.oauthIssuer);
  assert.deepEqual(metadata.code_challenge_methods_supported, ['S256']);
  assert.deepEqual(metadata.token_endpoint_auth_methods_supported, ['none']);
});

test('authorization form uses signed request state and token endpoint enforces one-time PKCE exchange', async (t) => {
  const { cfg, base } = await start(t);
  ...
  assert.equal(html.includes(cfg.allowedRedirectUris[0]), false);
  ...
  assert.equal(redirect.searchParams.get('state'), 'chatgpt-state-123');
  assert.equal(redirect.searchParams.get('iss'), cfg.oauthIssuer);
